...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupView(){
        val chats = (application as App).chats
        val additionalChats = (application as App).additionalChats
        ...
        recyclerView.adapter = ChatAdapter(chats, object : ChatAdapter.ChatClickListener {
            override fun onChatClick(chat: Chat, position: Int) {
                Toast.makeText(this@MainActivity, "Chat clicked", Toast.LENGTH_SHORT).show()
            }
        })
        ...
        val simpleItemTouchCallback = object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
                if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
                    ...
                    if (dX < 0) {
                        val archiveIcon = ContextCompat.getDrawable(this@MainActivity, R.drawable.ic_baseline_archive_24)
                        archiveIcon?.setTint(ContextCompat.getColor(this@MainActivity, R.color.white))
                        ...
                        archiveIcon.setBounds(deleteIconLeft.toInt(),
                            deleteIconTop.toInt(), deleteIconRight.toInt(), deleteIconBottom.toInt()
                        )
